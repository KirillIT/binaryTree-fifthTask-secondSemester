...
public class SimpleBSTree<T extends Comparable<? super T>> extends SimpleBinaryTree<T> implements BSTree<T> {
    ...
    public T put(T value, Ref1 ref) {
        ...
        return put(root, value, ref);
    }




    private T put(SimpleTreeNode node, T value, Ref1 ref) {
        ref.set(ref.get() + 1);
        int cmp = value.compareTo(node.value);
        if (cmp == 0) {
            // в узле значение, равное value
            ...
            if (cmp < 0) {
                if (node.left == null) {
                    ...
                    return put(node.left, value, ref);
                }
            } else {
                if (node.right == null) {
                    node.right = new SimpleTreeNode(value);
                    ...
                    return put(node.right, value, ref);
                }
            }
        }
    }
